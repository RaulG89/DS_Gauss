...
public class Server {
	...
	/**
	 * Crea un objeto con los parámetros de la petición. Para simplificar, no se
	 * hace validación de la entrada. NO ES OBJETIVO de este ejercicio mejorar
	 * ni generalizar esta parte del protocolo.
	 */
	...
	private void sendPage(Request request, Response response)
		...
		System.out.println(
				"[Log Server] Petición recibida: " + request.toString());
		...
		if (request.getPageURL().equals("index.html")) {
			...
			response.send("Estos son los pedidos realizados por -> "
					+ request.getUser());
