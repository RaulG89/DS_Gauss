...
import google.maps.*;
import google.maps.elements.MapMonument;
import google.maps.elements.MapPhoto;
import google.maps.elements.MapRestaurant;
import model.*;
...
public class Main {
	public static void main(String[] args) {
		...
		for (Monument monument : database.selectMonuments()) {
			map.add(new MapMonument(monument));
		}

		for (Photo photo : database.selectPhotos()) {
			map.add(new MapPhoto(photo));
		}

		for (Restaurant restaurant : database.selectRestaurants()) {
			map.add(new MapRestaurant(restaurant));
		}
		...
		System.out.println(
				"\n 3. Pulsación breve sobre cada elemento: se muestra un"
						+ " \"tooltip\"");
		...
		System.out.println(
				"\n 4. Pulsación larga sobre cada elemento: realiza la acción"
						+ " predeterminada con cada elemento");
