...
import google.maps.elements.MapElement;
...
public class Map {
	...
	public void draw() {
		...
		for (MapElement element : elements)
			System.out.println(
					element.getTitle() + " at " + element.getCoordinates());
	}
	...
	public void userTouch(int x, int y) {
		...
		if (element != null)
			System.out.println(
					"Se abre una ventana flotante: " + element.getHTMLInfo());
	}
	...
	private MapElement getElementAt(int x, int y) {
		for (MapElement element : elements) {
			...
			Rectangle elementArea = new Rectangle(
					...
					(int) coordinates.getLatitude() - 5, 10, 10);
