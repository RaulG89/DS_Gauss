...
import editor.tool.create.CreateRectangule;
import editor.tool.create.CreateTriangle;
import editor.tool.selection.SelectionTool;
...
public class Application {
	...
	private void askUser() throws IOException {
		...
		// pase ninguno (por usabilidad, para que el usuario se dé cuenta de que
		// la última
		...
		if (action.equals("salir")) {
			...
			editor.setTool(new CreateRectangule(editor));
		} else if (action.equals("circulo")) {
			editor.setTool(new CreateCircle(editor));
		} else if (action.equals("triangulo")) {
			editor.setTool(new CreateTriangle(editor));
		} else if (action.equals("pulsar")) {
			try {
				// la siguiente línea es para que funcione independientemente de
				// si las coordenadas
				// están separadas sólo por una coma o si hay espacios entre los
				// números y la coma
				String[] arguments = tokens[1].split("\\s*,\\s*");
				int x = Integer.parseInt(arguments[0]);
				int y = Integer.parseInt(arguments[1]);
				editor.getTool().pulse(x, y);
			} catch (NumberFormatException e) {
				output.println("Error de sintaxis: se esperaban las coordenadas"
						+ " del punto en que se hizo clic: pulsar <x>, <y>");
			}
